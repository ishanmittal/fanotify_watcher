 ...
 *
 *   Uses fanotify from kernel to report changes to files
 *
...
char open_buffer[MAXBUFLEN], modify_buffer[MAXBUFLEN];
char file_read[100];
int open_flag = 0;
...
static FILE * read_file_into_buffer(char * buffer, char * file_name)
{
 ...
 if (fp != NULL) {
     ...
     if ( ferror( fp ) != 0 ) {
         ...
         open_buffer[newLen++] = '\0'; /* Just to be safe. */
     }
...
event_process (struct fanotify_event_metadata *event)
{
  ...
  if (event->mask & FAN_OPEN) {
    if (event->pid != getpid())
    read_file_into_buffer(open_buffer, get_file_path_from_fd (event->fd, path, PATH_MAX));
    open_flag = 1;
    strcpy(file_read, get_file_path_from_fd (event->fd, path, PATH_MAX));
    		
    printf ("\t FAN_OPEN\n");
    printf ("\t File Content Read: %s\n", open_buffer);
  }
  ...
  if (event->mask & FAN_CLOSE_WRITE) {
    if (open_flag == 1 && !strcmp(file_read, get_file_path_from_fd (event->fd, path, PATH_MAX))) {
     if (event->pid != getpid())
      read_file_into_buffer(modify_buffer, get_file_path_from_fd (event->fd, path, PATH_MAX));
      file_read[0] = '\0';
      open_flag = 0;
   }
